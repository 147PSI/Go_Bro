...
def set_new_node(sgf_game, color, position):
    new_node = sgf_game.extend_main_sequence()
    new_node.set(color, position)
...
class GoRecord():
    ...
    def write_new_step_to_sgf_file(self, color, position):
        if self._check_sgf_file_existence():
            with open(self.record_storage_path + self.sessionId + ".sfg", "r") as f:
                ...
                sgf_string = set_new_node(g, color, position)
